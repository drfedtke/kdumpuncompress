...
# python3 has now cStringIO and changed 
# "print >> fh,..." into "fh.write(' '.join(..." below...
...
class ProtoTreeItem(PacketList):
    ...
    def dump(self, fh=sys.stdout):
        if self.name:
            ...
            fh.write(' '.join((" name=%s " % (quoteattr(self.name),),)))

        if self.showname:
            #print >> fh, "showname=%s" % (quoteattr(self.showname),),
            fh.write(' '.join(("showname=%s " % (quoteattr(self.showname),),)))

        if self.pos:
            #print >> fh, "pos=%s" % (quoteattr(self.pos),),
            fh.write(' '.join(("pos=%s " % (quoteattr(self.pos),),)))

        if self.size:
            #print >> fh, "size=%s" % (quoteattr(self.size),),
            fh.write(' '.join(("size=%s " % (quoteattr(self.size),),)))

        if self.value:
            #print >> fh, "value=%s" % (quoteattr(self.value),),
            fh.write(' '.join(("value=%s " % (quoteattr(self.value),),)))

        if self.show:
            #print >> fh, "show=%s" % (quoteattr(self.show),),
            fh.write(' '.join(("show=%s " % (quoteattr(self.show),),)))

        if self.hide:
            #print >> fh, "hide=%s" % (quoteattr(self.hide),),
            fh.write(' '.join(("hide=%s " % (quoteattr(self.hide),),)))

class Packet(ProtoTreeItem, PacketList):
    def dump(self, fh=sys.stdout, indent=0):
        #print >> fh, "  " * indent, "<packet>"
        fh.write(' '.join(("  " * indent, "<packet>"+"\n")))
        indent += 1
        for child in self.children:
